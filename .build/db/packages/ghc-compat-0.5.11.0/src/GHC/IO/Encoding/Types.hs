...
module GHC.IO.Encoding.Types (
    BufferCodec(..), getEncode, getRecover,
    TextEncoding(..),
...
getEncode :: BufferCodec from to state -> CodeBuffer from to
getEncode = encode

getRecover :: BufferCodec from to state -> (Buffer from -> Buffer to -> IO (Buffer from, Buffer to))
getRecover = recover
...
type CodingBuffer   from to = IO (CodingProgress, Buffer from, Buffer to)
