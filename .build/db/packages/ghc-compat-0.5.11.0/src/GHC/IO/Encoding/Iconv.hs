...
charIsRepresentable _ _ = return True  -- XXX this is nonsense
...
iconvRecode iconv_t
  ...
  = do
    ...
    with (fromIntegral ((os-ow) `shiftL` oscale)) $ \ p_outleft -> do
      ...
      errno <- getErrno
      case errno of
        e --XXX | e == e2BIG  -> return (OutputUnderflow, new_input, new_output)
          ...
          --XXX | e == eILSEQ -> return (if new_outleft' == 0 then OutputUnderflow else InvalidSequence, new_input, new_output)
          | otherwise -> do
